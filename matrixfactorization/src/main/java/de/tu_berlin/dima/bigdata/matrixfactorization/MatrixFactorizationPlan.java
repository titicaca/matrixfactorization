...
//	private final ReduceContract itemFeatureMatrixReducers[] = new ReduceContract[numIterations];
	...
	public Plan getPlan(String... args) {
		...
		CrossContract itemFeatureMatrixCrosser;
		ReduceContract itemFeatureMatrixReducer;
		
		for(int i = 1; i < numIterations; i ++){
//			System.out.println("iteration :" + i);
			itemFeatureMatrixCrosser = CrossContract
			...
			itemFeatureMatrixReducer = ReduceContract
					.builder(ItemFeatureMatrixReducer.class, PactInteger.class, 0).input(itemFeatureMatrixCrosser)
					.name("Item Feature Matrix Update Reducer " + (i)).build();
			
			userFeatureMatrixCrosser = CrossContract
					.builder(UserFeatureMatrixCrosser.class).input1(userRatingVectorReducer).input2(itemFeatureMatrixReducer)
					.name("User Feature Matrix Update Crosser " + (i+1)).build();
			
			userFeatureMatrixReducer = ReduceContract
					.builder(UserFeatureMatrixReducer.class, PactInteger.class, 0).input(userFeatureMatrixCrosser)
		...
		CrossContract predictCrosser = CrossContract.builder(PredictionCrosser.class)
				.input1(itemFeatureMatrixCrosser)
				.input2(userFeatureMatrixCrosser)
